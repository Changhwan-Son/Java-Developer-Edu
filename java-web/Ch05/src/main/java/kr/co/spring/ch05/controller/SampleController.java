...
public class SampleController {
	...
	@GetMapping("/ex03")
	public String ex03(Model model) {	// Model : 클라이언에게 전달할 데이터를 담는 클래스 
	...
	// 1) 생성자에 매개변수가 없어야 한다. (Default Constructor 만 사용해야 한다.)
	...
	@GetMapping("/ex04")
	public String ex04(
			SampleDTO sampleDTO, 	// Java Bean이 파라미터로 넘어오게 되면 addAttribute를 하지 않아도 자동으로 attribute로 등록이 된다. 
	...
	// 메소드의 리턴 타입이 void인 경우 
	// 메소드의 이름 자체가 jsp 파일 이름이 된다. 
	...
	// Rest API화 하기 
	// @RestController로 컨트롤러 클래스를 만들면 모든 함수가 Rest API화가 되지만, 
	// 일반 @Controller에서 Rest API를 만들기 위해서는
