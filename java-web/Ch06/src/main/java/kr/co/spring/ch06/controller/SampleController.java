...
public class SampleController {
	// Restful Controller는 MIME 타입을 직접 지정해서 클라이언트에게 보여주고자 하는 데이터의 형식을 지정할 수 있다. 
	...
	public List<SampleVO> getList() {
		...
		return IntStream.range(1,  10)
				.mapToObj(i -> new SampleVO(i, i + "firstName", i + "lastName"))
	...
	// @PathVariable
	// URL 형식으로 파라미터 담기 
	// 일반 @Controller와 @RestController 모두 지원하지만, RestController에서 사용하는 것이 대중적이다. (관례)
	// 보통은 RestFul 방식을 구현하기 위해서 많이 사용합니다. 
	
	// 주의 !!!!!!!! PathVariable을 사용해서 파라미터를 받을 때는 자바의 기본자료형이 들어갈 수 없습니다.
	// ex) double -> Double
	// 	   int    -> Integer
	...
	public String[] getPath(@PathVariable("cat")String cat, @PathVariable("pid")Integer pid) {
		...
		return new String[] {"catagory: " + cat, "product: "+ pid};
	}
